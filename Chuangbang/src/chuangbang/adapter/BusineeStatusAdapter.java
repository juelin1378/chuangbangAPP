...
public class BusineeStatusAdapter extends BaseAdapter{
	...
	public BusineeStatusAdapter(List<Project> data,Context context){
		if (data == null) {
	...
	public View getView(int position, View convertView, ViewGroup arg2) {
		...
		if(convertView==null){
			...
			holder.tvDomain=(TextView)convertView.findViewById(R.id.tv_business_domain);
			holder.tvDescription=(TextView)convertView.findViewById(R.id.tv_business_description);
			holder.tvDate=(TextView)convertView.findViewById(R.id.tv_business_date);
			holder.tvComment=(TextView)convertView.findViewById(R.id.tv_business_comments);
			holder.roundImage=(ImageView)convertView.findViewById(R.id.iv_businees_avater);
			holder.tvFavorite=(TextView)convertView.findViewById(R.id.tv_business_favorite);
			convertView.setTag(holder);
		}else{
			holder=(ViewHolder)convertView.getTag();
		}

		holder.tvName.setText("项目名称   ："+data.get(position).getName());
		holder.tvDate.setText(data.get(position).getCreatedAt());
		if(data.get(position).getDomain()!=null)
			holder.tvDomain.setText("项目领域  ： "+data.get(position).getDomain());
		...
		if(data.get(position).getFavoriteUserCount()!=null)
			holder.tvFavorite.setText(data.get(position).getFavoriteUserCount()+"收藏");
